...
(function(){
  ...
  theme.Images = (function() {
    ...
    function imageSize(src) {
      if (!src) {
        return '1000x'; // default based on theme
      }
      ...
      colorSwatchHovering: function() {
        ...
        this.container.querySelectorAll(selectors.colorSwatch).forEach(swatch => {
          swatch.addEventListener('mouseenter', () => {
            var ids = swatch.dataset.productId;
            var id = swatch.dataset.variantId;
            var image = swatch.dataset.variantImage;
            var el = this.container.querySelector('.grid-product__color-image--' + id);
            //el.style.backgroundImage = 'url(' + image + ')';
            el.classList.add('is-active');
            //$('[data-product-id='+ids + '] .grid-product__image-mask .image-wrap').css("opacity","0");
            $('.grid-product__color-image--' + id).siblings('.image-wrap').css("opacity","0");
            $('.grid-product__color-image--' + id).siblings().removeClass('is-active');
          });
          swatch.addEventListener('mouseleave', () => {
            var id = swatch.dataset.variantId;
            //this.container.querySelector('.grid-product__color-image--' + id).classList.remove('is-active');
            //this.container.querySelector('.grid-product__color-image--' + id).siblings().removeClass('is-active');
            //$('.grid-product__image-mask .image-wrap').css("opacity","1");
...
$(document).ready(function(){
  if ($(window).width() < 769) {
    $(".pro_icon_bottom").appendTo(".product-single__form");
}
    var clipboard = new Clipboard('.dis_code');
    clipboard.on('success', function(e) {
        $('.dis_code').html('Copied');
    });

});
$('.nav_fixed').scrollToFixed({marginTop: 63});
$('.p_nav li a').smoothScroll({
  offset:-150
});
$('.grid-product__colors a').mouseover(function() {
    $(this).addClass('active').siblings().removeClass('active');
  });
$('.tab_cpanel h2').click(function() { 
...
$('.pr_info h3').click(function() {
  $('.pr_info').removeClass('active');
  $(this).parent().addClass('active');
})
 $('.tab_nav ul li').click(function() {
      $('.tabspan .ts_item').css('display', 'none');
      $('.tab_nav ul li').removeClass('active');
      $('.tabspan .ts_item').eq($(this).index()).css('display', 'block');
      $(this).addClass('active');
    })
$('.pr_info h3').click(function() {
      $('.pr_text_right .pr_img').css('display', 'none');
      $('.pr_info h3').removeClass('active');
      $('.pr_text_right .pr_img').eq($(this).parent().index()).css('display', 'block');
      $(this).addClass('active');
    })
